...
def apply_gradcam_and_lime(image_path, model, target_size=(128, 128), opacity=0.4):
    ...
    heatmap, predictions = get_gradcam_heatmap(
        model, img_array_preprocessed, last_conv_layer_name)

    # Prediction probabilities
    class_names = ['Glioma', 'Meningioma', 'No tumor', 'Pituitary']
    confidences = {class_names[i]: predictions[0][i]
                   for i in range(len(class_names))}

    # LIME Explanation
    explainer = lime_image.LimeImageExplainer()
    explanation = explainer.explain_instance(
        img_array.astype('double'), predict_fn, top_labels=4, hide_color=0, num_samples=1000
    )
    ...
    axes[1, 0].barh(list(confidences.keys()), list(
        confidences.values()), color='skyblue')
    axes[1, 0].set_title('Prediction Confidence')
    axes[1, 0].set_xlabel('Probability')
    axes[1, 0].set_xlim(0, 1)

    # LIME Explanation Overlay
    lime_overlay_result = explanation.get_image_and_mask(
    ...
    """
    axes[1, 2].text(0, 0.5, explanation_text, ha='left',
                    va='center', fontsize=10, wrap=True, color='black')
